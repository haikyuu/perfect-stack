				...
				<div .w-full.max-w-md>
					...
					<form .mt-8.bg-white.rounded-lg.shadow-xl.overflow-hidden @submit.prevent=form.submit("post", "/login")>
						<div .px-10.py-12>
							...
							<text-input value=form.data.email error=form.errors.email label="Email" type="email" autofocus autocapitalize="off" .mt-10 />
							<text-input value=form.data.password error=form.errors.password .mt-6 label="Password" type="password" />
							<label .mt-6.select-none.flex.items-center for="remember">
								<input id="remember" value=form.data.remember .mr-1 type="checkbox" />
