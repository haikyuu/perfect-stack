		...
		<self .flex.items-center>
			<div .flex.w-full.bg-white.shadow.rounded>
				<dropdown auto-close=no .px-4.{'md:px-6'}.rounded-l.border-r.{'hover:bg-gray-100'}.{'focus:border-white'}.{'focus:ring'}.{'focus:z-10'} placement="bottom-start">
					...
					<div [max-width:{maxWidth}] slot="dropdown" .px-4.py-6.w-screen.shadow-xl.bg-white.rounded>
						<slot>
					
				
				<input.w-full.px-6.py-3.rounded-r.{'focus:ring'} autocomplete="off" type="text" name="search" placeholder="Search…" bind=data @input.debounce(200ms)=_change />
